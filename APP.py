...
# ---------------- Inductive Miner ----------------
if log is not None:
    st.subheader("📌 Ідеальний workflow:")
    st.markdown("Inductive → зрозуміти процес")
    st.markdown("Heuristics → побачити проблеми")
    st.markdown(" ")
    st.subheader("Типові патерни і що вони означають:")
    st.markdown("🔁 Багато циклів → rework, помилки, нестача інформації")
    st.markdown("🔀 Багато XOR → нестандартизований процес")
    st.markdown("🧱 Товста стрілка в одному місці → bottleneck")
    st.markdown("🔄 Петлі назад → перевірки, відмови, доопрацювання")
    st.markdown(" ")
    st.markdown(" ")
    st.subheader("Inductive Miner")
    st.markdown("Inductive Miner → Process Tree")
    st.markdown("Inductive Miner будує дерево процесу. Це логічна структура, а не «реальний граф виконання».")
    st.markdown(" ")
    st.markdown("Основні елементи Process Tree:")
    st.markdown("1. Листи (activities)")
    st.markdown("- Прямокутники / підписи з назвами активностей")
    st.markdown("- Це реальні дії з логів (Start, Approve, End тощо)")
    st.markdown("2. Оператори (вузли дерева). Вони зазвичай позначаються символами або словами:")
    st.markdown("- SEQ (→) — послідовність(це жорсткий порядок, без варіацій)")
    st.markdown("- ⊕ XOR — вибір (альтернатива). Типово: різні сценарії, різні типи кейсів")
    st.markdown("- ∧ AND — паралельність")
    st.markdown("- ⟳ LOOP — цикл. Типово: rework, повернення на доопрацювання")
    st.markdown(" ")
    st.markdown("🧩 Як читати Process Tree практично")
    st.markdown("1. Починай з кореня дерева")
    st.markdown("2. Запитай себе: Це послідовність? вибір? паралель?")
    st.markdown("3. Спускайся до листів (активностей). Формулюй речення: «Процес починається з …, далі або … або …, після чого …»")
    st.markdown(" ")
...
if log is not None:
    ...
    st.subheader("📤 Експорт схеми процесу")
    
    # --- Тимчасова директорія ---
    tmp_dir = tempfile.mkdtemp()
    
    # --- Шляхи ---
    png_path = os.path.join(tmp_dir, "process_graph")
...
# ---------------- Timeline кейсу ----------------
    ...
    if st.button("⬇️ Завантажити PDF звіт з графіками"):
        ...
        # Заголовок
        elements.append(Paragraph("Process Mining Report", header_style))
        elements.append(Paragraph("Автор AAP: Hanna Nesterenko | LinkedIn: https://www.linkedin.com/in/anna-nesterenko-bi/", text_style))
        elements.append(Spacer(1, 12))
    
        # --- Загальна статистика ---
        elements.append(Paragraph("📊 Загальна статистика", header_style))
        elements.append(Paragraph(f"Кількість кейсів: {num_cases}", text_style))
        elements.append(Paragraph(f"Середня тривалість кейсу: {avg_duration:.2f} год", text_style))
        elements.append(Paragraph(f"Середня кількість кроків на кейс: {avg_activities:.1f}", text_style))
        elements.append(Spacer(1, 12))
    
        # --- Rework ---
        elements.append(Paragraph("🔁 Повторювані кроки (Rework)", header_style))
        elements.append(Paragraph(f"Кейсів з повтореннями: {total_rework_cases} ({percent_rework}%)", text_style))
        elements.append(Paragraph(f"ТОП кроків з повтореннями: {', '.join(top_rework.index.tolist()[:5])}", text_style))
        elements.append(Spacer(1, 12))
    
        # --- Variant analysis ---
        elements.append(Paragraph("⚡ Variant analysis (ТОП 5 сценаріїв)", header_style))
        for i, row in variant_counts.iterrows():
            elements.append(Paragraph(f"{i+1}. {row['Сценарій процесу']} — {row['Кількість кейсів']} кейсів", text_style))
        elements.append(Spacer(1, 12))
        ...
        # --- Boxplot Lead Time ---
        boxplot_path = os.path.join(tempfile.mkdtemp(), "leadtime_boxplot.png")
        ...
        plt.close()
        elements.append(Paragraph("📈 Boxplot Lead Time", header_style))
        elements.append(RLImage(boxplot_path, width=450, height=200))
        elements.append(Spacer(1, 12))
    
        # --- Heuristics Miner Graph ---
        heuristics_path = os.path.join(tempfile.mkdtemp(), "heuristics_graph.png")
        dot.format = "png"
        dot.render(heuristics_path, cleanup=True)
        elements.append(Paragraph("🔥 Heuristics Miner", header_style))
        elements.append(RLImage(heuristics_path + ".png", width=450, height=300))
        elements.append(Spacer(1, 12))
        ...
        st.download_button(
            label="Завантажити PDF звіт з графіками",
            data=buffer,
            file_name="process_mining_report.pdf",
            mime="application/pdf"
        )
